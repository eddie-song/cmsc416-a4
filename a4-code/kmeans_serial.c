// kmeans_serial.c: Serial K-means clustering in C.
// C port of the provided kmeans.py program.
//
// Usage: ./kmeans_serial <datafile> <nclust> [savedir] [maxiter]
...
int main(int argc, char **argv){
  ...
  while(nchanges > 0 && curiter <= MAXITER){
    ...
    for(int c = 0; c < nclust; c++){
      for(int d = 0; d < data.dim; d++){
        clust.features[c * data.dim + d] = 0.0;
      }
    }
    ...
    for(int i = 0; i < data.ndata; i++){
      ...
      for(int d = 0; d < data.dim; d++){
        clust.features[c * data.dim + d] += data.features[i * data.dim + d];
      }
    }
